...
CPing::CPing() :m_szICMPData(NULL), m_bIsInitSucc(FALSE)
{
	...
	else
	{
		...
		m_szICMPData = (char*)malloc(DEF_PACKET_SIZE_0 + sizeof(ICMPHeader));
...
BOOL CPing::PingCore(DWORD dwDestIP, PingReply *pPingReply, DWORD dwTimeout)
{
	...
	//构建ICMP包
	int nICMPDataSize = DEF_PACKET_SIZE_0 + sizeof(ICMPHeader);
	ULONG ulSendTimestamp = GetTickCountCalibrate();
	USHORT usSeq = ++s_usPacketSeq;
	memset(m_szICMPData, 0, nICMPDataSize);
	ICMPHeader *pICMPHeader = (ICMPHeader*)m_szICMPData;
	pICMPHeader->m_byType = ECHO_REQUEST_0;
			...
			if (netEvent.lNetworkEvents & FD_READ)
			{
				...
				if (nPacketSize != SOCKET_ERROR)
				{
					...
					if (pICMPHeader->m_usID == m_usCurrentProcID //是当前进程发出的报文
						&& pICMPHeader->m_byType == ECHO_REPLY_0 //是ICMP响应报文
